...
#import "DCCJSONRequestSerializer.h"
#import "DCCQueryParamCollection.h"
#import "DCCDefaultConfiguration.h"

NSString *const DCCResponseObjectErrorKey = @"DCCResponseObject";

static NSString * const kDCCContentDispositionKey = @"Content-Disposition";

static NSDictionary * DCC__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * DCC__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = DCC__headerFieldsForResponse(response);
    if(!headers[kDCCContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kDCCContentDispositionKey];
...
@interface DCCApiClient ()

@property (nonatomic, strong, readwrite) id<DCCConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation DCCApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static DCCApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[DCCDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[DCCDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithConfiguration:(id<DCCConfiguration>)configuration {
    return [self initWithBaseURL:[NSURL URLWithString:configuration.host] configuration:configuration];
}

- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<DCCConfiguration>)configuration {
    self = [super initWithBaseURL:url];
    if (self) {
        _configuration = configuration;
        _timeoutInterval = 60;
        _responseDeserializer = [[DCCResponseDeserializer alloc] init];
        _sanitizer = [[DCCSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        DCCJSONRequestSerializer * swgjsonRequestSerializer = [DCCJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kDCCApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        DCCDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[DCCResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = DCC__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<DCCSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = DCCPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = DCCPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, DCCPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[DCCQueryParamCollection class]]){
            DCCQueryParamCollection * coll = (DCCQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = DCCPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = DCCPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = DCCPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<DCCConfiguration> config = self.configuration;
