...
@protocol DCCDataCollectorBeginTransactionRequest
@end

@interface DCCDataCollectorBeginTransactionRequest : DCCDataCollectorBaseRequest

/* Timeout (in seconds) for the transaction [optional]
 */
@property(nonatomic) NSNumber* timeout;
/* Timeout mode for the transaction. With TXN, the timeout is reset when an update is posted to the same transaction. With ANY, the timeout is reset when an update is posted for any transaction w/ the same user/device [optional]
 */
@property(nonatomic) NSString* timeoutMode;
